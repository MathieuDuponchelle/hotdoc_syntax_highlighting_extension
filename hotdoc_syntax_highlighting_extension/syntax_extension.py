...
class SyntaxHighlightingExtension(BaseExtension):
    ...
    def __formatting_page_cb(self, formatter, page):
        ...
        page.output_attrs['html']['scripts'].add(
            os.path.join(HERE, 'prism', 'plugins', 'autoloader', 'prism-autoloader.js'))
        page.output_attrs['html']['scripts'].add(
            os.path.join(HERE, 'prism_autoloader_path_override.js'))

        folder = os.path.join(formatter.get_output_folder(), 'assets', 'prism_components')
    ...
    def add_arguments(parser):
        group = parser.add_argument_group('Syntax highlighting extension',
                DESCRIPTION)
        group.add_argument('--syntax-highlighting-activate', action="store_true",
                help="Activate the syntax highlighting extension", dest='syntax_highlighting_activate')

    @staticmethod
    def parse_config(doc_repo, config):
        SyntaxHighlightingExtension.activated = bool(config.get('syntax_highlighting_activate', False))
